...
# Final workflow
final_tree <- finalize_workflow(tree_wf, best_tree_acc)

# Last fit and metrics
...
# Final workflow
final_nnet <- finalize_workflow(nnet_wf, best_nnet_acc)
...
# Get best accuracy and finalize workflow
...
# What is the best value of cost and rbf_sigma?
top_svm_acc  <- show_best(rec_res_svm, metric = "accuracy")
best_svm_acc <- select_best(rec_res_svm, metric = "accuracy")
final_svm <- finalize_workflow(rec_wf_svm, best_svm_acc)
...
datasummary_df(all_ans %>% select(-.metric,-.estimator,-mean,-n,-std_err),output="latex",fmt = '%.4f') %>% print